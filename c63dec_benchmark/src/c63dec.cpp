...
const char *c63dec_bench = "VirtioFS0/c63dec_bench.csv";
const char *csv_header = "run_number,time_ms\n";
const size_t csv_header_len = strlen(csv_header);
...
void write_result(int results_fd, const struct BenchmarkResult& result) {
  std::string results_str  = std::format("{},{:.3f}\n",
    result.run_number,
    result.time_ms
  );
  ...
  int run_number)
{
  ...
  unlink("VirtioFS0/wwe.yuv");
  FILE *fin = fopen("VirtioFS0/wwe.c63", "rb");
  FILE *fout = fopen("VirtioFS0/wwe.yuv", "wb");
  ...
  result.time_ms = start_stop_diff.count() * 1000.0;
...
int main()
{
  int results_fd = open(c63dec_bench, O_CREAT|O_WRONLY, 777);
  if (results_fd < 0) {
  ...
  for (int run = 0; run < 30; ++run) {
    c63dec_benchmark(results_fd, cm, run);
  }
